...
//    }
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning)
    {
        ...
        let snapshotView = fromVC.selectedCell.pictureImageView.snapshotView(afterScreenUpdates: false)
        snapshotView?.frame = container.convert(fromVC.selectedCell.pictureImageView.frame, from: fromVC.selectedCell)
        fromVC.selectedCell.pictureImageView.isHidden = true
        ...
        UIView.animate(withDuration: transitionDuration(using: transitionContext), delay: 0, options: UIViewAnimationOptions(), animations:{ () -> Void in
            ...
            fromVC.selectedCell.pictureImageView.isHidden = false
